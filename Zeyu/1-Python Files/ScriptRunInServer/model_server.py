...
class DATA(config_server.Config):
    ...
    def stackdata(self, ChooseSensors=['IMU','GONIO']):
        ...
        if self.IfSplitData:
            print('Start stacking data...(Split data into train set and val set), and split ratio is ', config_server.Config.SPILTRATIO)
            for i in range(len(self.__DATAFileName)):
                for j in range(len(self.__DATAFileName[i])):
                    ...
                    if i==0 and j ==0:
                        CUTPoint = int(len(array_data1)*(1-config_server.Config.SPILTRATIO))#split the data, cutpoint is a integer.
